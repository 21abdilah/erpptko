  ...
  <div class="container py-3">
    ...
    <div class="row g-3 mb-4">
      <div class="col-6 col-md-3" v-for="card in summaryCards" :key="card.title">
        <div class="p-3 rounded shadow-sm h-100 d-flex justify-content-between align-items-center"
             :class="card.bg">
          <div>
    ...
    <div class="table-responsive shadow-sm rounded mb-3">
      <table class="table table-striped table-bordered table-hover align-middle mb-0">
        ...
        <tbody>
          <tr v-for="s in paginatedData" :key="s.sale_id + '-' + s.item_name">
            ...
            <td class="text-end">
              ...
              <div v-if="s.paid > 0">
                Sudah bayar: <small class="text-success">{{ formatCurrency(s.paid) }}</small>
              </div>
              <div>
                Sisa: <small class="text-danger">{{ formatCurrency(s.remaining) }}</small>
              </div>
            </td>
            ...
            <td>
              <span class="badge rounded-pill"
                    :class="{
                      ...
                      'bg-warning text-dark': getStatusBadge(s)==='unpaid',
                      'bg-orange text-dark': getStatusBadge(s)==='partial'
                    }">
                {{ getStatusLabel(s) }}
              </span>
            </td>
...
const paginatedData = computed(()=>history.value.slice((page.value-1)*perPage, page.value*perPage))
